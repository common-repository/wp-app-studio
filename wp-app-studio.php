...
/*
   ...
   Description: Wp App Studio is a Professional WordPress Plugin Development Platform for building commercial grade WordPress plugins. No coding required.
   Version: 6.1.1
   Author: eMarket Design LLC
   Author URI: http://emarketdesign.com
   Text Domain: wp-app-studio
...
define('WPAS_URL', "https://wpappstudio.com");
define('WPAS_SSL_URL', "https://api.emarketdesign.com");
define ('WPAS_ADDON_URL','https://emd-plugin-site.s3.amazonaws.com/');
...
require_once("views/connection_edd.php");
require_once("views/connection_form.php");
require_once("views/addon_pages.php");
require_once("views/glob_form.php");
require_once("views/licenses.php");
require_once("lib/wpas_admin_funcs.php");

load_plugin_textdomain( 'wp-app-studio', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
...
function wpas_activate () 
{
      ...
      set_transient( '_wpas_activation_redirect', true, 30 );
}
...
add_action( 'admin_init', 'wpas_update', 12);
...
function wpas_plugin_menu() {
	global $hook_app_list, $hook_app_new, $hook_generate, $hook_licenses, $hook_store, $hook_design, $hook_support, $hook_debug;
	...
	add_submenu_page('wpas_app_list', __( 'Getting Started', 'wp-app-studio'),__( 'Getting Started', 'wp-app-studio'),'design_wpas','wpas-getting-started','wpas_getting_started_page');
	$hook_app_new = add_submenu_page( 'wpas_app_list', __('Add New App','wp-app-studio'), __('Add New App','wp-app-studio'), 'design_wpas', 'wpas_add_new_app', 'wpas_add_new_app');
	$hook_licenses = add_submenu_page( 'wpas_app_list', __('Licenses','wp-app-studio'), __('Licenses','wp-app-studio'), 'design_wpas', 'wpas_licenses_page', 'wpas_licenses_page');
	$hook_generate = add_submenu_page( 'wpas_app_list', __('Generate','wp-app-studio'), __('Generate','wp-app-studio'), 'design_wpas', 'wpas_generate_page', 'wpas_generate_page');
	$hook_store = add_submenu_page( 'wpas_app_list', __('Plugins','wp-app-studio'), __('Plugins','wp-app-studio'), 'design_wpas', 'wpas_store_page', 'wpas_store_page');
	$hook_design = add_submenu_page( 'wpas_app_list', __('Designs','wp-app-studio'), __('Designs','wp-app-studio'), 'design_wpas', 'wpas_design_page', 'wpas_design_page');
	$hook_support = add_submenu_page( 'wpas_app_list', __('Support','wp-app-studio'), __('Support','wp-app-studio'), 'design_wpas', 'wpas_support_page', 'wpas_support_page');
	$hook_debug = add_submenu_page( 'wpas_app_list', __('Debug','wp-app-studio'), __('Debug','wp-app-studio'), 'design_wpas', 'wpas_debug_page', 'wpas_debug_page');
	add_action( 'admin_enqueue_scripts', 'wpas_enqueue_scripts' );

}
function wpas_enqueue_scripts($hook_suffix){
	global $hook_app_list, $hook_app_new, $hook_generate, $hook_licenses, $hook_store, $hook_design, $hook_support, $hook_debug, $local_vars,$form_vars, $layout_vars, $validate_vars;
	if($hook_suffix == $hook_app_list)
	{
		...
		$local_vars['nonce_duplicate'] = wp_create_nonce( 'wpas_duplicate_nonce' );
	...
	elseif($hook_suffix == $hook_app_new)
	{
		...
		$local_vars['nonce_create_def_views'] = wp_create_nonce( 'wpas_create_def_views_nonce' );
		...
		wp_enqueue_style("jq-css",plugin_dir_url( __FILE__ ) . 'css/jquery-ui.min.css');
		wp_enqueue_script('jquery-ui-tooltip');
		...
		wp_enqueue_style('codemirror-css',plugin_dir_url( __FILE__ ) . 'css/codemirror.min.css');
		wp_enqueue_script('codemirror-js',plugin_dir_url( __FILE__ ) . 'js/codemirror/codemirror.min.js');
		wp_enqueue_script('codemirror-css-js',plugin_dir_url( __FILE__ ) . 'js/codemirror/css.min.js');
		wp_enqueue_script('codemirror-xml-js',plugin_dir_url( __FILE__ ) . 'js/codemirror/xml.min.js');
		wp_enqueue_script('codemirror-javascript-js',plugin_dir_url( __FILE__ ) . 'js/codemirror/javascript.min.js');
		wp_enqueue_script('codemirror-htmlmixed-js',plugin_dir_url( __FILE__ ) . 'js/codemirror/htmlmixed.js');
		wp_enqueue_script('codemirror-clike-js',plugin_dir_url( __FILE__ ) . 'js/codemirror/clike.js');
		wp_enqueue_script('codemirror-php-js',plugin_dir_url( __FILE__ ) . 'js/codemirror/php.js');
		wp_enqueue_script('codemirror-autorefresh-js',plugin_dir_url( __FILE__ ) . 'js/codemirror/autorefresh.js');
	}
	...
	elseif($hook_suffix == $hook_licenses){
		wp_enqueue_style('wpas-admin-css', plugin_dir_url( __FILE__ ) . 'css/wpas-admin.css');
		wp_enqueue_style('wpas-core',plugin_dir_url( __FILE__ ) . 'css/wpas-core.css');
		wp_enqueue_script('wpas-lic',plugin_dir_url( __FILE__ ) . 'js/wpas-license.js');
	}
...
function wpas_update()
{
	...
	if($current_wpas_version != WPAS_VERSION)
	{
		...
		if(version_compare($current_wpas_version,'5.3.5','<')) {
			$new_wpas_licenses = Array();
			$wpas_licenses = get_option('wpas_licenses',Array());
			$wpas_license_status = get_option('wpas_license_status',Array());
			$wpas_license_error = get_option('wpas_license_error',Array());
			if(!empty($wpas_licenses['edition']) && !empty($wpas_licenses['edition']['key'])){
				$key = $wpas_licenses['edition']['key'];
				$new_wpas_licenses['edition'][$key]['name'] = $wpas_licenses['edition']['name'];		
				if(!empty($wpas_license_status[$key]) && $wpas_license_status[$key]['type'] == 'edition'){
					$new_wpas_licenses['edition'][$key]['status'] = $wpas_license_status[$key]['status'];
					unset($wpas_license_status[$key]);
				}
				if(!empty($wpas_license_error[$key])){
					$new_wpas_licenses['edition'][$key]['error'] = $wpas_license_error[$key];
					unset($wpas_license_error[$key]);
				}
				if(!empty($wpas_license_status)){
					foreach($wpas_license_status as $lic_key => $lic_status){
						if($lic_status['type'] == 'edition'){
							$new_wpas_licenses['edition'][$lic_key]['name'] = $lic_status['name'];
							$new_wpas_licenses['edition'][$lic_key]['status'] = $lic_status['status'];
							if(!empty($wpas_license_error[$lic_key])){
								$new_wpas_licenses['edition'][$lic_key]['error'] = $wpas_license_error[$lic_key];
							}
						}
					}
				}
			}
			if(!empty($wpas_licenses['connection'])){
				foreach($wpas_licenses['connection'] as $lic_conn){
					if(!empty($lic_conn['key'])){
						$ckey = $lic_conn['key'];
						$new_wpas_licenses['connection'][$ckey]['name'] = $lic_conn['name'];
						if(!empty($wpas_license_status[$ckey]) && $wpas_license_status[$ckey]['type'] == 'conn'){
							$new_wpas_licenses['connection'][$ckey]['status'] = $wpas_license_status[$ckey]['status'];		
						}
						if(!empty($wpas_license_error[$ckey])){
							$new_wpas_licenses['connection'][$ckey]['error'] = $wpas_license_error[$ckey];	
						}
					}	
				}	
			}	
			if(!empty($new_wpas_licenses)){
				update_option('wpas_licenses',$new_wpas_licenses);
			}
		}	
		//if(version_compare($current_wpas_version,'4.6.0','<=')) {
		...
		//}
		if($current_wpas_version == '5.0.0'){
			$licenses = get_option('wpas_licenses',Array());
			if(!empty($licenses)){
				foreach($licenses as $mylic){
					if($mylic['name'] == 'freedev'){
						$new_lics['edition']['name'] = 'freedev';
						$new_lics['edition']['key'] = $mylic['key'];
					}
				}
				if(!empty($new_lics)){
					update_option('wpas_licenses',$new_lics);
				}
				delete_option('wpas_license_error');
			}
		}
		delete_user_meta(get_current_user_id(), 'wpas_dismiss_docs');
		delete_user_meta(get_current_user_id(), 'wpas_dismiss_buynow');
		update_option('wpas_version',WPAS_VERSION);
		wpas_add_design_cap();
	}
	if(isset($_POST['Save']) && sanitize_text_field($_POST['Save']) == 'Rename')
	{
		wpas_is_allowed();
		...
		$app_name = stripslashes(sanitize_text_field($_POST['app_title']));
...
function wpas_add_new_app()
{
	...
	//saving an app	
	if(isset($_POST['type']) && sanitize_text_field($_POST['type']) == 'app' && sanitize_text_field($_POST['Save']) == "Save")
	{
...
function wpas_show_page($app,$page)
{
	...
	if($page == "edit_app")
	{
		...
		echo "<div id=\"loading\" class=\"group1\" style=\"display: none;\">" . esc_html__("Please wait","wp-app-studio") . "...</div>";
		wpas_modal_confirm_delete();
...
function wpas_modal_confirm_delete($generate=0)
{
	...
	echo "<div class=\"modal hide\" id=\"" . $modal_id . "\">
                ...
                <h3><i class=\"icon-trash icon-red\"></i>" . esc_html__("Delete","wp-app-studio") . "</h3>
                </div>
                <div class=\"modal-body\" style=\"clear:both\">";
	if($generate == 1){
		 esc_html_e("Are you sure you wish to delete all your generation log history?","wp-app-studio");
	}
	elseif($generate == 2){
		 esc_html_e("Inline Entities can not have attributes. Checking inline entity option will remove all your existing attributes for this entity. Are you sure?","wp-app-studio");
	}
	else {
		 esc_html_e("Are you sure you wish to delete?","wp-app-studio");
	}
	echo "</div>
                <div class=\"modal-footer\">
                <button id=\"" . $cancel_mid . "\" class=\"btn btn-danger\" data-dismiss=\"" . $modal_id . "\" aria-hidden=\"true\">" . esc_html__("Cancel","wp-app-studio") . "</button> 
                <button id=\"" . $delete_mid . "\" data-dismiss=\"" . $modal_id . "\" aria-hidden=\"true\" class=\"btn btn-primary\">" . esc_html__("OK","wp-app-studio") . "</button>
                </div>
                </div>";
}
...
?>
	<div class="updated">
                  <?php
                    printf(
                        '<p> %1$s <a href="' . WPAS_URL . '/wp-app-studio-knowledge-center/?pk_campaign=wpas&pk_source=plugin&pk_medium=link&pk_content=doc-notice" target="_blank"> %2$s </a> %3$s <a style="float:right;" href="%4$s"><span class="dashicons dashicons-dismiss" style="font-size:15px;"></span>%5$s</a></p>',
                        __( 'See our updated', 'wp-app-studio' ),
                        __( 'Documentation', 'wp-app-studio' ),
                        __( '&#187;', 'wp-app-studio' ),
                        esc_url( add_query_arg( 'wpas_dismiss_docs', true ) ),
                        __( 'Dismiss', 'wp-app-studio' )
                    );
                    ?>
                </div>
<?php
	...
	?>
			<div class="updated">
			<p><a href="<?php echo WPAS_URL; ?>/wp-app-studio-pricing/?pk_campaign=wpas&pk_source=plugin&pk_medium=link&pk_content=pricing-notice" target="_blank">See our New Development Plans and start developing your own WordPress plugin now! &#187;</a>
			<a style="float:right;" href="<?php echo esc_url( add_query_arg( 'wpas_dismiss_buynow', true )); ?>"><span class="dashicons dashicons-dismiss" style="font-size:15px;"></span><?php esc_html_e( 'Dismiss', 'wp-app-studio' ); ?></a>
			</p>
			</div>
	<?php
		}
