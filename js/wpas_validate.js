jQuery(document).ready(function($) {
		var unique;
		// reserved words for entity and taxonomy and app
		var arr_app = ['try','post','page'];
		var arr_ent = ['post','page','attachment','revision','nav_menu_item','comments','license','notifications','incoming_email','single_taxonomy','walker_radio','query','widget','comment_data','comments_list_table','analytics','chart','datagrid','entity', 'try', 'afc_columns','afc_filters','report','notify_email','ent_tax_order','session','calendar','inline_entity','rating_log'];
		var arr_txn = ['attachment','attachment_id','author','author_name','calendar','cat','category','category__and','category__in','category__not_in','category_name','comments_per_page','comments_popup','cpage','day','debug','error','exact','feed','hour','link_category','m','minute','monthnum','more','name','nav_menu','nopaging','offset','order','orderby','p','page','page_id','paged','pagename','pb','perm','post','post__in','post__not_in','post_format','post_mime_type','post_status','post_tag','post_type','posts','posts_per_archive_page','posts_per_page','preview','robots','s','search','second','sentence','showposts','static','subpost','subpost_id','tag','tag__and','tag__in','tag__not_in','tag_id','tag_slug__and','tag_slug__in','taxonomy','tb','term','type','w','withcomments','withoutcomments','year','try'];
		$.validator.addMethod('checkWooTerm', function(value, element) {
			if(value == '' && $('#connection-woo_tax').val() != ''){
				return false;
			}
			return true;
		}, validate_vars.nowoo_term);	
		$.validator.addMethod('checkWooTax', function(value, element) {
			if(value == '' && $('#connection-woo_order_rel').prop('checked') && $('#connection-woo_product_rel').prop('checked')){
			return false;
			}
			return true;
		}, validate_vars.nowoo_tax);	
		$.validator.addMethod('checkEddTerm', function(value, element) {
			if(value == '' && $('#connection-edd_tax').val() != ''){
				return false;
			}
			return true;
		}, validate_vars.noedd_term);	
		$.validator.addMethod('checkEddTax', function(value, element) {
			if(value == '' && $('#connection-edd_order_rel').prop('checked') && $('#connection-edd_product_rel').prop('checked')){
			return false;
			}
			return true;
		}, validate_vars.noedd_tax);	
		...
		$.validator.addMethod('noReservedApp', function(value, element) { 
			if($.inArray(value,arr_app) == -1)
			{
				if(value.match(/^emd_/) == null){
					return true;
				}
			}
			return false;
		...
		$.validator.addMethod('checkNotify', function(value, element) {
			if(!$('#notify-email_admin_confirm').prop('checked') && !$('#notify-email_user_confirm').prop('checked'))
			{
				return false;
			}
			return true;
		...
		$.validator.addMethod('checkDefault', function(value, element) { 
			...
			{
				var is_multiple = $('#fld_multiple').prop('checked');
		...
		$.validator.addMethod('checkFontValues', function(value, element) { 
			if(value.indexOf(';;') >= 0 && value != '')
			{
				return false;
			}
			...
			$.each(input, function(key,inp) {
				...
				if(inp.length != 0)
				{
					if(inp.match(/\.(eot|svg|ttf|woff)$/) == null){
							return valid = false;
					}
				}
			});
			return valid;
		}, validate_vars.check_font_values);
		...
		$.validator.addMethod('checkAppDash',function(value,element){
			var resp = true;
			if($('#shc-app_dash').prop('checked') && $('#shc-view_type').val() == 'integration')
			{
		...
		$.validator.addMethod('checkDash',function(value,element){
			if(!$('#widg-app_dash').prop('checked') && !$('#widg-wp_dash').prop('checked'))
			{
				return false;
			}
			return true;
		...
		$('#connection-form').validate(
		{
			...
			'connection-calendar_label':{
			required:true,
			},
			'connection-calendar_title':{
			required:true,
			},
			'connection-calendar_start':{
			required:true,
			},
			'connection-calendar_end':{
			required:true,
			},
			'connection-rating_admin_loc':{
			required:true,
			},
			'connection-rating_label':{
			required:true,
			},
			'connection-woo_label':{
			required:true,
			},
			'connection-woo_order_layout':{
			required:true,
			},
			'connection-woo_product_layout':{
			required:true,
			},
			'connection-woo_product_term':{
			checkWooTerm:true,
			},
			'connection-woo_order_term':{
			checkWooTerm:true,
			},
			'connection-woo_tax':{
			checkWooTax:true,
			},
			'connection-edd_label':{
			required:true,
			},
			'connection-edd_order_layout':{
			required:true,
			},
			'connection-edd_product_layout':{
			required:true,
			},
			'connection-edd_product_term':{
			checkEddTerm:true,
			},
			'connection-edd_order_term':{
			checkEddTerm:true,
			},
			'connection-edd_tax':{
			checkEddTax:true,
			},
			'connection-mailchimp_label':{
			required:true,
			},
			'connection-mailchimp_form':{
			required:true,
			},
			},
			success: function(label) {
