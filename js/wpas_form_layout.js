jQuery(document).ready(function($) {
	...
	$(document).on('change','.form-element-select',function(){
		...
		select_label = "";
			...
			else if($(this).find('option:selected').attr('ftype') == 'custom')
			{
				$(this).closest('.form-'+type).find('#field-label'+label_id).removeClass('elmt btn-std').addClass('custom-fld');
				$(this).closest('.form-inside .row-fluid').find('.add-element').hide();
			}	
			else
			{
	...
	$(document).on('change','.form-element-size',function(){
		label_vars = $(this).attr('id').split('-');
		type = label_vars[1];
		label_id = label_vars[4];
                if($(this).find('option:selected').val() != '')
